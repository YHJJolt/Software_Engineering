<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="StudentCalendar.aspx.cs" 
         Inherits="SchoolSystem.Student.StudentCalendar" MasterPageFile="~/Student/StudentMaster.Master" %>
...
<div class="calendar-page">
    ...
    <div id="eventModal" class="modal">
        <div class="modal-content">
            ...
            <input type="hidden" id="eventType" value="Personal" />
            ...
            <div id="typePicker" class="type-tags-container">
                <div class="type-tag active" data-type="Personal">Personal</div>
                <div class="type-tag" data-type="Study">Study</div>
                <div class="type-tag" data-type="Assignment">Assignment</div>
                <div class="type-tag" data-type="Exam">Exam</div>
                <div class="type-tag" data-type="Reminder">Reminder</div>
            </div>
...
var calendar;
const typeColorMap = {
    "Personal": "#6366f1", "Study": "#4ade80", "Assignment": "#f97316",
    "Exam": "#f87171", "Reminder": "#60a5fa"
};
...
function showSidebar(dateStr) {
    ...
    if (dayEvents.length === 0) {
        ...
        const colorStyles = {
            ...
            "Study": { bg: "#dcfce7", text: "#4ade80" },
            "Assignment": { bg: "#ffedd5", text: "#f97316" },
            "Exam": { bg: "#fee2e2", text: "#f87171" },
            "Reminder": { bg: "#dbeafe", text: "#60a5fa" }
        };

        dayEvents.forEach(event => {
            const type = event.extendedProps.type || "Personal";
            const style = colorStyles[type] || colorStyles["Personal"];
...
$(document).ready(function () {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: function (fetchInfo, successCallback) {
            $.when(
                $.ajax({ url: 'StudentCalendar.aspx/GetEvents', type: 'POST', contentType: 'application/json', data: '{}' }),
                $.ajax({ url: 'StudentCalendar.aspx/GetAdminEvents', type: 'POST', contentType: 'application/json', data: '{}' })
            ).done(function (sRes, aRes) {
                const studentEvents = sRes[0].d || [];
                const adminEvents = Array.isArray(aRes[0].d) ? aRes[0].d : [];

                const allEvents = studentEvents.concat(adminEvents).map(event => {
...
function saveEvent() {
    ...
    $.ajax({
        url: 'StudentCalendar.aspx/AddEvent',
        type: 'POST',
...
function openEditModal(id) {
    ...
    var savedType = eventObj.extendedProps.type || "Personal";
...
function updateEvent() {
    ...
    $.ajax({
        url: 'StudentCalendar.aspx/UpdateEvent',
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify({
...
function deleteEvent(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'StudentCalendar.aspx/DeleteEvent',
                type: 'POST',
