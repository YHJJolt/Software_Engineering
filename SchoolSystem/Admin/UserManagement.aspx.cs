...
namespace SchoolSystem
{
    public partial class UserManagement : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string form = Request.Form["__EVENTTARGET"] ?? "";
                bool isPending = form == btnDoDelete.UniqueID || form == btnDoToggle.UniqueID || form == btnDoResetPw.UniqueID;
                if (!isPending &&
                    !form.Contains("gvStudents") && !form.Contains("gvLecturers"))
                {
                    hfPendingAction.Value = "";
                }
            }
        }
        ...
        private void WriteDropdownData()
        {
            ...
            litDropdownData.Text =
                "<script>var _progData='" + string.Join("||", progs) +
                "'; var _deptData='" + string.Join("||", depts) + "';</script>";
        }
        ...
        protected void btnAdvanceSession_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(
                        "UPDATE CourseAssignment_Session SET assign_status='Inactive' WHERE academic_session=@s AND assign_status='Active'", conn))
                    { cmd.Parameters.AddWithValue("@s", current); cmd.ExecuteNonQuery(); }

                    string next = GetNextSession(current);
                    using (SqlCommand cmd = new SqlCommand(
                        "UPDATE SystemSettings SET setting_value=@v WHERE setting_key='current_session'", conn))
                    { cmd.Parameters.AddWithValue("@v", next); cmd.ExecuteNonQuery(); }
                }

                LoadStudents(); LoadAllStatistics();
                ScriptManager.RegisterStartupScript(this, GetType(), "advOk",
                    "Swal.fire({title:'Session Advanced',text:'Session advanced and graduations processed successfully.',icon:'success',confirmButtonColor:'#C5A059'});", true);
            }
            catch (Exception ex)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "advErr",
                    "Swal.fire({title:'Error',text:'" + JsStr(ex.Message) + "',icon:'error',confirmButtonColor:'#C5A059'});", true);
            }
        }

        private string GetNextSession(string session)
        {
            if (string.IsNullOrWhiteSpace(session) || session.Length < 7) return "JAN2026";
        ...
        private void LoadProgramAdd()
        {
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                if (!string.IsNullOrEmpty(selected) && ddlProgramAdd.Items.FindByValue(selected) != null)
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                new SqlDataAdapter(new SqlCommand(@"
                    SELECT s.student_id, s.student_code, s.student_name, s.student_email,
                           ...
                           s.student_isactive, s.student_sem, p.program_name
                    FROM [Student] s
        ...
        protected void btnAddStudent_Click(object sender, EventArgs e)
        {
            ...
            if (missing.Count > 0)
            {
                ...
                LoadStudents(); LoadAllStatistics(); return;
            }
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    SqlCommand cmd = new SqlCommand(@"
                        ...
                        VALUES ('TMP',@name,'stud123','placeholder',
                             NULLIF(@contact,''),NULL,NULL,@sem,'Active',1,@prog);
                    ...
                    new SqlCommand("UPDATE [Student] SET student_code=@c,student_email=@e WHERE student_id=@id", conn)
                    { Parameters = { new SqlParameter("@c", code), new SqlParameter("@e", email), new SqlParameter("@id", realId) } }
                    .ExecuteNonQuery();
                }
            }
            catch (Exception ex)
        ...
        protected void btnAddLecturer_Click(object sender, EventArgs e)
        {
            ...
            if (missing.Count > 0)
            {
                ...
                LoadLecturers(); LoadAllStatistics(); return;
            }
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    SqlCommand cmd = new SqlCommand(@"
                        ...
                        VALUES ('TMP',@name,'lect123','placeholder',
                             NULLIF(@contact,''),NULL,NULL,@dept,'Active',1);
                        SELECT CAST(SCOPE_IDENTITY() AS INT);", conn);
                    cmd.Parameters.AddWithValue("@name", name);
                    cmd.Parameters.AddWithValue("@contact", contact);
                    ...
                    new SqlCommand("UPDATE [Lecturer] SET lecturer_code=@c,lecturer_email=@e WHERE lecturer_id=@id", conn)
                    { Parameters = { new SqlParameter("@c", code), new SqlParameter("@e", email), new SqlParameter("@id", realId) } }
                    .ExecuteNonQuery();
                }
            }
        ...
        // ══════════════════════════════════════════════════════════════
        // HIDDEN BUTTON HANDLERS — fired after SweetAlert confirmation
        // ══════════════════════════════════════════════════════════════
        protected void btnDoDelete_Click(object sender, EventArgs e)
        {
            int id = Convert.ToInt32(hfPendingId.Value);
            string type = hfPendingType.Value; // "student" or "lecturer"
            hfPendingAction.Value = "";
            ...
            if (type == "student")
            {
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    new SqlCommand(@"DELETE ar FROM [AttendanceRecord] ar
                        INNER JOIN [Enrollment] e ON ar.enrollment_id=e.enrollment_id WHERE e.student_id=@id", conn)
                    { Parameters = { new SqlParameter("@id", id) } }.ExecuteNonQuery();

                    new SqlCommand(@"DELETE pd FROM [PaymentDetail] pd
                        INNER JOIN [Enrollment] e ON pd.enrollment_id=e.enrollment_id WHERE e.student_id=@id", conn)
                    { Parameters = { new SqlParameter("@id", id) } }.ExecuteNonQuery();

                    new SqlCommand("DELETE FROM [AssignmentSubmission] WHERE student_id=@id", conn)
                    { Parameters = { new SqlParameter("@id", id) } }.ExecuteNonQuery();

                    new SqlCommand(@"DELETE cg FROM [CourseGrade] cg
                        INNER JOIN [Enrollment] e ON cg.Enrollment_id=e.enrollment_id WHERE e.student_id=@id", conn)
                    { Parameters = { new SqlParameter("@id", id) } }.ExecuteNonQuery();
            ...
            else // lecturer
            {
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                        ...
                        "SELECT COUNT(1) FROM [Program] WHERE Lecturer_id=@id", conn))
                    {
                        ...
                        {
                            SetToast("✗ Cannot delete: lecturer is set as Head of Program. Reassign first.", "error");
                            LoadLecturers(); LoadAllStatistics(); return;
                        }
                    }
        ...
        protected void btnDoToggle_Click(object sender, EventArgs e)
        {
            int id = Convert.ToInt32(hfPendingId.Value);
            string type = hfPendingType.Value;
            hfPendingAction.Value = "";
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                if (type == "student")
                {
                    ...
                    { Parameters = { new SqlParameter("@id", id) } }.ExecuteNonQuery();
                    SetToast("✓ Student status updated.", "success");
                    LoadStudents(); LoadAllStatistics();
                }
                else
                {
                    ...
                    { Parameters = { new SqlParameter("@id", id) } }.ExecuteNonQuery();
                    SetToast("✓ Lecturer status updated.", "success");
                    LoadLecturers(); LoadAllStatistics();
                }
            }
        }

        protected void btnDoResetPw_Click(object sender, EventArgs e)
        {
            int id = Convert.ToInt32(hfPendingId.Value);
            string type = hfPendingType.Value;
            string defpw = type == "student" ? "stud123" : "lect123";
            hfPendingAction.Value = "";
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                conn.Open();
                if (type == "student")
                    new SqlCommand("UPDATE [Student] SET student_pw=@pw WHERE student_id=@id", conn)
                    { Parameters = { new SqlParameter("@pw", defpw), new SqlParameter("@id", id) } }.ExecuteNonQuery();
                else
                    new SqlCommand("UPDATE [Lecturer] SET lecturer_pw=@pw WHERE lecturer_id=@id", conn)
                    { Parameters = { new SqlParameter("@pw", defpw), new SqlParameter("@id", id) } }.ExecuteNonQuery();
            }
            SetToast("✓ Password reset to default (" + defpw + ").", "success");
        ...
        protected void gvStudents_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "ReqDelete":
                    {
                        var parts = arg.Split('|');
                        hfPendingAction.Value = "delete";
                        hfPendingId.Value = parts[0];
                        hfPendingName.Value = parts.Length > 1 ? parts[1] : "";
                        hfPendingCode.Value = parts.Length > 2 ? parts[2] : "";
                        hfPendingType.Value = parts.Length > 3 ? parts[3] : "student";
                        hfToastMsg.Value = ""; hfToastType.Value = "";
                        break;
                    }
                case "ReqToggle":
                    {
                        var parts = arg.Split('|');
                        hfPendingAction.Value = "toggle";
                        hfPendingId.Value = parts[0];
                        hfPendingName.Value = parts.Length > 1 ? parts[1] : "";
                        hfPendingCode.Value = parts.Length > 2 ? parts[2] : "";
                        hfPendingStatus.Value = parts.Length > 3 ? parts[3] : "";
                        hfPendingType.Value = parts.Length > 4 ? parts[4] : "student";
                        hfToastMsg.Value = ""; hfToastType.Value = "";
                        break;
                    }
                case "ReqResetPw":
                ...
                case "ViewS":
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(connStr))
                        {
                            ...
                            SELECT s.student_id,s.student_code,s.student_name,s.student_email,
                                   ...
                                   s.date_of_birth,s.student_sem,s.student_isactive,p.program_name
                            FROM [Student] s
                            LEFT JOIN [Program] p ON s.Program_id=p.program_id
                            ...
                            using (SqlDataReader rdr = cmd.ExecuteReader())
                            {
                                if (rdr.Read())
                                {
                                    ...
                                    hfViewDOB.Value = rdr["date_of_birth"] != DBNull.Value
                                        ? Convert.ToDateTime(rdr["date_of_birth"]).ToString("dd MMM yyyy") : "N/A";
                ...
                case "ReqDelete":
                    {
                        ...
                        hfPendingName.Value = parts.Length > 1 ? parts[1] : "";
                        hfPendingCode.Value = parts.Length > 2 ? parts[2] : "";
                        hfPendingType.Value = parts.Length > 3 ? parts[3] : "lecturer";
                        hfToastMsg.Value = ""; hfToastType.Value = "";
                        break;
                    }
                case "ReqResetPw":
                ...
                case "ViewL":
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(connStr))
                        {
                            ...
                            SELECT lecturer_id,lecturer_code,lecturer_name,lecturer_email,
                                   ...
                                   date_of_birth,lecturer_department,teacher_isactive
                            ...
                            using (SqlDataReader rdr = cmd.ExecuteReader())
                            {
                                if (rdr.Read())
                                {
                                    ...
                                    hfViewDOB.Value = rdr["date_of_birth"] != DBNull.Value
                                        ? rdr["date_of_birth"].ToString() : "N/A";
