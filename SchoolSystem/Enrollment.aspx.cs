...
namespace SchoolSystem
{
    public partial class Enrollment : System.Web.UI.Page
    {
        ...
        private void LoadEnrollments(string searchTerm = "", string programId = "", string status = "")
        {
            lblMessage.Visible = false;
            lblMessage.Text = "";

            DataTable dt = GetEnrollments(searchTerm, programId, status);
        ...
        private DataTable GetEnrollments(string search = "", string programId = "", string status = "")
        {
            DataTable dt = new DataTable();
            bool isLecturer = hdnActiveTab.Value == "Lecturers";
            ...
            using (SqlConnection conn = new SqlConnection(ConnStr))
            {
                ...
                if (isLecturer)
                {
                    sql = @"SELECT le.lecturer_enrollment_id as enrollment_id, 
                                   l.lecturer_name as name, l.lecturer_email as email, 
                                   'N/A' as program_name, c.course_code, c.course_name, 
                                   le.enrollment_date, le.status 
                            FROM LecturerEnrollment le
                            INNER JOIN Lecturer l ON le.lecturer_id = l.lecturer_id
                            INNER JOIN Course c ON le.course_id = c.course_id
                            WHERE 1=1 ";
                }
                else
                {
                    sql = @"SELECT e.enrollment_id, s.student_name as name, s.student_email as email, 
                ...
                {
                    sql += isLecturer ? " AND l.lecturer_name LIKE @Search " : " AND s.student_name LIKE @Search ";
                }

                if (!string.IsNullOrEmpty(status))
                {
                    sql += " AND " + (isLecturer ? "le.status = @Status" : "e.status = @Status");
                }

                sql += " ORDER BY " + (isLecturer ? "l.lecturer_name" : "s.student_name");
        ...
        protected void Tab_Click(object sender, EventArgs e)
        {
            ...
            hdnActiveTab.Value = btn.CommandArgument;

            tabStudents.CssClass = "tab-btn" + (hdnActiveTab.Value == "Students" ? " tab-active" : "");
            tabLecturers.CssClass = "tab-btn" + (hdnActiveTab.Value == "Lecturers" ? " tab-active" : "");
        ...
        protected void BtnDeleteEnrollment_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(ConnStr))
                {
                    ...
                    if (isLecturer)
                    {
                        sql = "DELETE FROM LecturerEnrollment WHERE lecturer_enrollment_id = @ID";
                    }
                    else
                    {
                        // FIXED: Safely cascade delete CourseGrade before Enrollment
                        sql = @"
        ...
        {
            bool isLecturer = hdnActiveTab.Value == "Lecturers";
            string sql = isLecturer
                ? "UPDATE LecturerEnrollment SET status = @Status WHERE lecturer_enrollment_id = @ID"
                : "UPDATE Enrollment SET status = @Status WHERE enrollment_id = @ID";
                    ...
                    foreach (GridViewRow row in Enrollments.Rows)
                    {
                        ...
                        if (chkSelect != null && chkSelect.Checked)
                        {
                            ...
                            if (hdnId != null && !string.IsNullOrEmpty(hdnId.Value))
                            {
                                ...
                                {
                                    // FIXED: Safely cascade delete CourseGrade before Enrollment in BULK mode
                                    sql = @"
                                        DELETE FROM CourseGrade WHERE Enrollment_id = @ID;
                                        DELETE FROM Enrollment WHERE enrollment_id = @ID;";
                                }
